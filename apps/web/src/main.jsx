...
function AgroAguiarPresentationPage({ language }) {
  ...
  return <article className="guimarpeixe-recipes agro-presentation"><section className="gp-recipes-hero agro-presentation-hero"><Link className="back-link" to="/industrias/agroaguiar/produtos">{copy.back}</Link><div className="gp-products-brand"><img src="/brand/agroaguiar.jpeg" alt="AgroAguiar" /></div><p className="section-label">AgroAguiar / {language === 'en' ? 'Presentation' : language === 'es' ? 'Presentación' : 'Apresentação'}</p><h1>{copy.title.split('\n').map((line, index) => <span key={line}>{line}{index === 0 && <br />}</span>)}</h1><p>{copy.lead}</p></section><section className="agro-presentation-list"><article className="agro-presentation-card"><div className="agro-presentation-image"><img src="/brand/agro-products/agro-13.webp" alt="Mix de frutos vermelhos AgroAguiar" /><span>01</span></div><div className="agro-presentation-copy"><h2>AgroAguiar</h2><dl><div><dt>{copy.varieties}</dt><dd>{language === 'en' ? 'Frozen, organic and dried products' : language === 'es' ? 'Productos congelados, orgánicos y secos' : 'Produtos ultracongelados, orgânicos e secos'}</dd></div><div><dt>{copy.calibre}</dt><dd>{language === 'en' ? 'Various' : language === 'es' ? 'Varios' : 'Vários'}</dd></div><div><dt>{copy.conservation}</dt><dd>{language === 'en' ? 'According to product' : language === 'es' ? 'Según el producto' : 'Conforme o produto'}</dd></div></dl><button className="gp-recipe-open agro-video-trigger" type="button" onClick={() => setIsVideoOpen(true)}>{copy.video} <b>→</b></button></div></article></section>{isVideoOpen && <div className="agro-video-modal" role="dialog" aria-modal="true" aria-labelledby="agro-video-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setIsVideoOpen(false) }}><div className="agro-video-dialog"><header><div><p className="section-label">AgroAguiar / Vídeo</p><h2 id="agro-video-title">{copy.video}</h2></div><button className="agro-video-close" type="button" onClick={() => setIsVideoOpen(false)} aria-label={copy.close}>×</button></header><video controls autoPlay playsInline><source src="/brand/video-agroaguiar.mp4" type="video/mp4" />O seu navegador não suporta vídeo HTML5.</video><footer><span>AgroAguiar</span><button type="button" onClick={() => setIsVideoOpen(false)}>{copy.close}</button></footer></div></div>}</article>
}
