...
function AgroAguiarPresentationPage({ language }) {
  const copy = language === 'en' ? { title: 'AgroAguiar\npresentation.', lead: 'Discover the AgroAguiar range in video.', varieties: 'Varieties', calibre: 'Calibre', conservation: 'Storage', video: 'View video', back: '← Back to products' } : language === 'es' ? { title: 'Presentación\nAgroAguiar.', lead: 'Descubra la gama AgroAguiar en vídeo.', varieties: 'Variedades', calibre: 'Calibre', conservation: 'Conservación', video: 'Ver vídeo', back: '← Volver a productos' } : { title: 'Apresentação\nAgroAguiar.', lead: 'Conheça a gama AgroAguiar em vídeo.', varieties: 'Variedades', calibre: 'Calibre', conservation: 'Conservação', video: 'Ver vídeo', back: '← Voltar aos produtos' }
  return <article className="guimarpeixe-recipes agro-presentation"><section className="gp-recipes-hero agro-presentation-hero"><Link className="back-link" to="/industrias/agroaguiar/produtos">{copy.back}</Link><div className="gp-products-brand"><img src="/brand/agroaguiar.jpeg" alt="AgroAguiar" /></div><p className="section-label">AgroAguiar / {language === 'en' ? 'Presentation' : language === 'es' ? 'Presentación' : 'Apresentação'}</p><h1>{copy.title.split('\n').map((line, index) => <span key={line}>{line}{index === 0 && <br />}</span>)}</h1><p>{copy.lead}</p></section><section className="agro-presentation-list"><article className="agro-presentation-card"><div className="agro-presentation-image"><img src="/brand/agro-products/agro-13.webp" alt="Mix de frutos vermelhos AgroAguiar" /><span>01</span></div><div className="agro-presentation-copy"><h2>AgroAguiar</h2><dl><div><dt>{copy.varieties}</dt><dd>{language === 'en' ? 'Frozen, organic and dried products' : language === 'es' ? 'Productos congelados, orgánicos y secos' : 'Produtos ultracongelados, orgânicos e secos'}</dd></div><div><dt>{copy.calibre}</dt><dd>{language === 'en' ? 'Various' : language === 'es' ? 'Varios' : 'Vários'}</dd></div><div><dt>{copy.conservation}</dt><dd>{language === 'en' ? 'According to product' : language === 'es' ? 'Según el producto' : 'Conforme o produto'}</dd></div></dl><a className="gp-recipe-open" href="/brand/video-agroaguiar.mp4" target="_blank" rel="noreferrer">{copy.video} <b>↗</b></a></div></article></section></article>
}
...
function IndustryContentPage({ language }) {
  ...
  const sectionName = industrySlug === 'agroaguiar' && section === 'receitas' ? language === 'en' ? 'Presentation' : language === 'es' ? 'Presentación' : 'Apresentação' : c.industries.actions[section]
