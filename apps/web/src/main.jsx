...
import { BrowserRouter, Link, NavLink, Navigate, Route, Routes, useLocation, useParams, useSearchParams } from 'react-router-dom'
import './styles.css'
...
const industryDirectory = [
  { slug: 'guimarpeixe', name: 'Guimarpeixe', image: '/brand/guimarpeixe.svg', description: 'Seleção de peixes congelados de alta qualidade, diretamente de Portugal.' },
  { slug: 'superfish', name: 'Superfish', image: '/brand/superfish.png', description: 'Especialistas em bacalhau salgado seco, com qualidade reconhecida.' },
...
const agroAguiarProductGroups = [
  { title: 'Frutas e vegetais ultracongelados', pages: [
    ['Framboesa ultracongelada', 4], ['Morango ultracongelado', 5], ['Mirtilo ultracongelado', 6], ['Mirtilo silvestre ultracongelado', 7], ['Amora ultracongelada', 8], ['Amora silvestre ultracongelada', 9], ['Groselha vermelha ultracongelada', 10], ['Groselha preta ultracongelada', 11], ['Baga do sabugueiro ultracongelada', 12], ['Mix de frutos vermelhos ultracongelados', 13], ['Cereja ultracongelada', 14], ['Ginja ultracongelada', 15], ['Figo verde ultracongelado', 16], ['Figo preto ultracongelado', 17], ['Ameixa ultracongelada', 18], ['Abóbora ultracongelada', 19], ['Castanha pelada ultracongelada', 20], ['Castanha pelada ultracongelada partida', 21], ['Castanha assada ultracongelada', 22]
  ] },
  { title: 'Frutas orgânicas ultracongeladas', pages: [
    ['Morango ultracongelado orgânico', 24], ['Framboesa ultracongelada orgânica', 25], ['Mirtilo ultracongelado orgânico', 26], ['Amora ultracongelada orgânica', 27], ['Mix de frutos vermelhos ultracongelados orgânico', 28], ['Figo verde ultracongelado orgânico', 29], ['Figo preto ultracongelado orgânico', 30], ['Castanha pelada ultracongelada orgânica', 31]
  ] },
  { title: 'Frutos secos', pages: [['Castanha', 33], ['Snack castanha', 34], ['Figo seco', 35]] }
]

const agroAguiarPresentations = [
  { name: { pt: 'Framboesa', en: 'Raspberry', es: 'Frambuesa' }, image: '/brand/agro-products/agro-04.webp', video: '/brand/Framboesa.mp4', variety: "Driscoll's, Kweli, Tulameen e Pacific", calibre: '95-5, 70-30, 60-40, 50-50, partida' },
  { name: { pt: 'Morango', en: 'Strawberry', es: 'Fresa' }, image: '/brand/agro-products/agro-05.webp', video: '/brand/Morango.mp4', variety: 'Amandine, Camarosa, Victory, Flavia e Festival', calibre: '15-25, 25-35, +35' },
  { name: { pt: 'Mirtilo', en: 'Blueberry', es: 'Arándano' }, image: '/brand/agro-products/agro-06.webp', video: '/brand/Mirtilo.mp4', variety: 'Ozarkblue e Duke', calibre: 'N/A' },
  { name: { pt: 'Mirtilo Silvestre', en: 'Wild Blueberry', es: 'Arándano Silvestre' }, image: '/brand/agro-products/agro-07.webp', video: '/brand/Mirtilo Silvestre.mp4', variety: 'N/A', calibre: 'N/A' },
  { name: { pt: 'Amora', en: 'Blackberry', es: 'Mora' }, image: '/brand/agro-products/agro-08.webp', video: '/brand/Amora.mp4', variety: 'Tupi, Triple Crown e Chester', calibre: 'N/A' },
  { name: { pt: 'Groselha Vermelha', en: 'Red Currant', es: 'Grosella Roja' }, image: '/brand/agro-products/agro-10.webp', video: '/brand/Groselha vermelha.mp4', variety: 'Junifer', calibre: 'N/A' },
  { name: { pt: 'Groselha Preta', en: 'Blackcurrant', es: 'Grosella Negra' }, image: '/brand/agro-products/agro-11.webp', video: '/brand/Groselha Preta.mp4', variety: 'Wellington e Andega', calibre: 'N/A' },
  { name: { pt: 'Mix de Frutos Vermelhos', en: 'Red Berry Mix', es: 'Mezcla de Frutos Rojos' }, image: '/brand/agro-products/agro-13.webp', video: '/brand/Mix Frutos Vermelhos.mp4', variety: 'Morangos, amoras, framboesas, groselha vermelha, groselha preta e mirtilos', calibre: 'N/A' },
  { name: { pt: 'Cereja', en: 'Cherry', es: 'Cereza' }, image: '/brand/agro-products/agro-14.webp', video: '/brand/Cereja.mp4', variety: 'Lapin, Bing, Starking e Ambrunes', calibre: 'Inteira e sem caroço' },
  { name: { pt: 'Ginja', en: 'Sour Cherry', es: 'Guinda' }, image: '/brand/agro-products/agro-15.webp', video: '/brand/Ginja.mp4', variety: 'Lutovka e Nadwislanka', calibre: 'Inteira, metades e sem caroço' },
  { name: { pt: 'Figo Pingo Mel', en: 'Pingo Mel Fig', es: 'Higo Pingo Mel' }, image: '/brand/agro-products/agro-16.webp', video: '/brand/Figo_Pingo_Mel.mp4', variety: 'Pingo mel', calibre: 'Inteiro' },
  { name: { pt: 'Ameixa', en: 'Plum', es: 'Ciruela' }, image: '/brand/agro-products/agro-18.webp', video: '/brand/Ameixa.mp4', variety: 'Stanley', calibre: 'Metades e sem caroço' }
]

const frigostoOvenProducts = [
  { name: 'Bolinho com Bacalhau', image: '/brand/bolinho-com-bacalhau.jpg', description: 'Produto ultracongelado, com a forma tradicional, formado por uma mistura homogénea à base de bacalhau.', weight: '40 g ou 60 g' },
  { name: 'Empadas com Bacalhau', image: '/brand/empadas-com-bacalhau.jpg', description: 'Produto ultracongelado produzido manualmente, com formato tradicional, constituído com recheio à base de bacalhau.', weight: '55 g ou 85 g' },
  { name: 'Rissóis com Cogumelos Shitake', image: '/brand/rissois-com-cogumelos-shitake.jpg', description: 'Produto ultracongelado em forma de meia-lua, com recheio à base de cogumelos Shitake e vegetais envolvido por uma cobertura de massa, revestida uniformemente com pão ralado.', weight: '70 g' },
  { name: 'Bacalhau com Natas', image: '/brand/bacalhau-com-natas.jpg', description: 'Produto ultracongelado e pré-cozinhado obtido da mistura homogénea do bacalhau com vários ingredientes.', weight: 'Dose individual: 380 g' },
  { name: 'Bacalhau à Brás', image: '/brand/bacalhau-a-bras.jpg', description: 'Produto ultracongelado e pré-cozinhado, constituído por uma camada inferior (mistura homogénea dos vários ingredientes) e uma camada superior de batata.', weight: 'Dose individual: 300 g' }
]
...
pageContent.pt.industries.text = 'Agenciamento de indústrias alimentares para empresas que pretendem internacionalizar-se através da exportação para o Brasil.'
...
const industryTranslations = {
  ...
  agroaguiar: { en: 'Portuguese agri-food industry specialising in delicious and sustainable fruit.', es: 'Agroindustria portuguesa especialista en frutas deliciosas y sostenibles.' },
...
const gelpinhosProductContent = {
  eyebrow: 'Gelpinhos / Produtos',
  title: 'Gelpinhos.',
  intro: 'Uma seleção de bacalhau dessalgado ultracongelado, preparada para combinar conveniência, qualidade e formatos adequados a cada operação.',
  listLabel: 'Bacalhau dessalgado ultracongelado',
  listTitle: 'Uma linha completa para cada necessidade.',
  catalogueLabel: 'Catálogo virtual',
  catalogueTitle: 'Consulte a gama completa.',
  catalogueText: 'Veja o catálogo Gelpinhos para conhecer todos os formatos, pesos e especificações da linha.',
  download: 'Abrir catálogo PDF',
  description: 'Descrição do produto',
  origin: 'Origem',
  table: ['Formato', 'Peso', 'Embalagem', 'Paletização'],
  products: [
    { name: 'Posta Longa Asa Branca', species: 'Gadus Morhua', origin: 'Noruega / Islândia', note: 'Granel / Higienizado', images: ['/brand/gelp1.webp'], rows: [['Longa Jumbo', '1000 g+', '9 kg', '54 / Pal.'], ['Longa Extra', '700 g - 1000 g', '9 kg', '54 / Pal.'], ['Longa', '500 g - 800 g', '9 kg', '54 / Pal.']] },
    { name: 'Lombos de Bacalhau', species: 'Gadus Morhua', origin: 'Noruega / Islândia', note: 'Granel / Higienizado / Caixa', images: ['/brand/gelp2.webp'], rows: [['Granel', '200 g - 300 g', '9 kg', '54 / Pal.'], ['Granel', '—', '800 g', '54 / Pal.'], ['Granel', '300 g - 400 g', '9 kg', '54 / Pal.'], ['Granel', '400 g - 500 g', '9 kg', '54 / Pal.'], ['Granel', '500 g+', '9 kg', '54 / Pal.'], ['Caixa 800 g', '—', '6,4 kg', '54 / Pal.']] },
    { name: 'Postas de Bacalhau', species: 'Gadus Morhua', origin: 'Noruega / Islândia', note: 'Granel / Higienizado', images: ['/brand/gelp3.webp'], rows: [['Posta longa', '500 g - 800 g', '9 kg', '54 / Pal.'], ['Posta média', '300 g - 500 g', '9 kg', '54 / Pal.'], ['Posta pequena', '200 g - 300 g', '9 kg', '54 / Pal.'], ['Posta pequena', '150 g - 200 g', '9 kg', '54 / Pal.']] },
    { name: 'Bacalhau Desfiado', species: 'Gadus Morhua / Gadus Macrocephalus', note: 'Avulso / Bolsa', images: ['/brand/gelp4.webp'], rows: [['Granel', '—', '6,4 kg', '54 / Pal.'], ['Bolsa 800 g', '—', '6,4 kg', '54 / Pal.']] },
    { name: 'Desfiado de Alasca Polak', species: 'Gadus Chalcogrammus', note: 'Avulso / Bolsa', images: ['/brand/gelp5.webp'], rows: [['Granel', '—', '6,4 kg', '54 / Pal.'], ['Bolsa 800 g', '—', '6,4 kg', '54 / Pal.']] },
    { name: 'Lombos de Bacalhau', species: 'Gadus Morhua', origin: 'Noruega / Islândia', images: ['/brand/gelp6.webp'], rows: [['Granel / Higienizado', '200 g - 300 g', '10 kg', '54 / Pal.'], ['Granel / Higienizado', '300 g - 400 g', '10 kg', '54 / Pal.'], ['Granel / Higienizado', '400 g - 600 g', '10 kg', '54 / Pal.'], ['Caixinha 800 g', '800 g', '6,4 kg', '54 / Pal.']] },
    { name: 'Lombos de Bacalhau Sem Espinhas', species: 'Gadus Morhua', origin: 'Noruega / Islândia', images: ['/brand/gelp7.webp'], rows: [['Granel', '—', '10 kg / 6 un.', '54 / Pal.'], ['Caixinha 800 g', '—', '6,4 kg', '54 / Pal.']] },
    { name: 'Postas de Bacalhau', species: 'Gadus Morhua', origin: 'Noruega / Islândia', note: 'Granel / Higienizado', images: ['/brand/gelp8-1.webp', '/brand/gelp8-2.webp'], rows: [['Posta com lombo', '300 g - 500 g', '10 kg', '54 / Pal.'], ['Posta com lombo', '500 g - 800 g', '10 kg', '54 / Pal.'], ['Posta pequena', '200 g - 300 g', '10 kg', '54 / Pal.'], ['Posta tradicional', '200 g - 300 g', '10 kg', '54 / Pal.']] },
    { name: 'Bacalhau Desfiado', species: 'Gadus Morhua / Gadus Macrocephalus', origin: 'Noruega / Islândia / Pacífico', note: 'Avulso / Bolsa', images: ['/brand/gelp9.webp'], rows: [['Granel', '—', '8 kg', '54 / Pal.'], ['Bolsa 800 g', '—', '6,4 kg', '54 / Pal.']] },
    { name: 'Desfiado de Alasca Polak', species: 'Gadus Chalcogrammus', origin: 'Noruega / Islândia / Pacífico', note: 'Avulso / Bolsa', images: ['/brand/gelp10.webp'], rows: [['Granel', '—', '8 kg', '54 / Pal.'], ['Bolsa 800 g', '—', '6,4 kg', '54 / Pal.']] }
  ]
}

const gelpinhosProducts = {
  pt: gelpinhosProductContent,
  en: { ...gelpinhosProductContent, eyebrow: 'Gelpinhos / Products', intro: 'A selection of frozen desalted cod, prepared to combine convenience, quality and formats suited to every operation.', listLabel: 'Frozen desalted cod', listTitle: 'A complete line for every need.', catalogueLabel: 'Digital catalogue', catalogueTitle: 'View the complete range.', catalogueText: 'See the Gelpinhos catalogue for every format, weight and product specification.', download: 'Open PDF catalogue', description: 'Product description', origin: 'Origin', table: ['Format', 'Weight', 'Packaging', 'Palletisation'] },
  es: { ...gelpinhosProductContent, eyebrow: 'Gelpinhos / Productos', intro: 'Una selección de bacalao desalado ultracongelado, preparada para combinar conveniencia, calidad y formatos adecuados para cada operación.', listLabel: 'Bacalao desalado ultracongelado', listTitle: 'Una línea completa para cada necesidad.', catalogueLabel: 'Catálogo virtual', catalogueTitle: 'Consulte la gama completa.', catalogueText: 'Consulte el catálogo Gelpinhos para conocer todos los formatos, pesos y especificaciones de la línea.', download: 'Abrir catálogo PDF', description: 'Descripción del producto', origin: 'Origen', table: ['Formato', 'Peso', 'Embalaje', 'Paletización'] }
}
...
function SearchPanel({ onClose, t, language }) {
  ...
  const gelpinhos = gelpinhosProducts[language]
  ...
  const entries = [
    ...
    { to: '/industrias/gelpinhos/produtos', title: `Gelpinhos · ${gelpinhos.listLabel}`, type: resultTypes.industry, content: `${gelpinhos.intro} ${gelpinhos.products.map((product) => `${product.name} ${product.species} ${product.origin || ''} ${product.rows.flat().join(' ')}`).join(' ')}` },
    ...
    { to: '/industrias/gelpinhos/receitas', title: `Gelpinhos · ${gelpinhosRecipes[language].eyebrow.split(' / ')[1]}`, type: resultTypes.industry, content: `${gelpinhosRecipes[language].lead} ${gelpinhosRecipes[language].recipes.map((recipe) => `${recipe.name} ${recipe.difficulty} ${recipe.time}`).join(' ')}` },
...
function GelpinhosProductsPage({ language }) {
  const c = pageContent[language]
  const content = gelpinhosProducts[language]
  const recipesTitle = language === 'en' ? 'Bring cod to the table.' : language === 'es' ? 'Más bacalao en la mesa.' : 'Mais bacalhau à mesa.'
  const recipesText = language === 'en' ? 'Discover seven cod suggestions from Gelpinhos.' : language === 'es' ? 'Descubra siete sugerencias con bacalao Gelpinhos.' : 'Descubra sete sugestões com bacalhau Gelpinhos.'
  const recipesLink = language === 'en' ? 'Explore recipes' : language === 'es' ? 'Explorar recetas' : 'Explorar receitas'
  return <article className="superfish-products gelpinhos-products"><section className="superfish-products-hero"><Link className="back-link" to="/industrias">{c.industries.back}</Link><div className="gp-products-brand"><img src="/brand/gelpinhos-azul.jpeg" alt="Gelpinhos" /></div><p className="section-label">{content.eyebrow}</p><h1>{content.title}</h1><p>{content.intro}</p></section><section className="superfish-products-list"><header><p className="section-label">{content.listLabel}</p><h2>{content.listTitle}</h2></header><div className="superfish-product-stack">{content.products.map((product, index) => <article className="superfish-product" key={`${product.name}-${index}`}><header><span>{String(index + 1).padStart(2, '0')}</span><h3>{product.name}</h3></header><div className="superfish-product-details"><div className="superfish-description"><p className="section-label">{content.description}</p><p>{product.species}</p>{product.origin && <p><b>{content.origin}:</b> {product.origin}</p>}{product.note && <p>{product.note}</p>}</div><div className="superfish-table-wrap"><table><thead><tr>{content.table.map((heading) => <th key={heading}>{heading}</th>)}</tr></thead><tbody>{product.rows.map((row) => <tr key={row.join('-')}>{row.map((cell, cellIndex) => <td key={`${cell}-${cellIndex}`}>{cell}</td>)}</tr>)}</tbody></table></div></div><ProductCarousel images={product.images} alt={product.name} /></article>)}</div></section><section className="gp-recipes-prompt superfish-recipes-prompt"><div><p className="section-label">Gelpinhos / {language === 'en' ? 'Recipes' : language === 'es' ? 'Recetas' : 'Receitas'}</p><h2>{recipesTitle}</h2><p>{recipesText}</p></div><Link to="/industrias/gelpinhos/receitas">{recipesLink} <span>→</span></Link></section><section className="superfish-catalogue"><div><p className="section-label">{content.catalogueLabel}</p><h2>{content.catalogueTitle}</h2><p>{content.catalogueText}</p></div><a href="/brand/Catalogo-Gelpinhos.pdf" target="_blank" rel="noreferrer">{content.download} <span>↗</span></a></section></article>
}
...
function AgroAguiarPresentationPage({ language }) {
  ...
  const copy = language === 'en' ? { title: 'AgroAguiar\npresentation.', lead: 'Discover the AgroAguiar range in video.', varieties: 'Varieties', calibre: 'Calibre', conservation: 'Storage', video: 'View video', close: 'Close video', back: '← Back to products' } : language === 'es' ? { title: 'Presentación\nAgroAguiar.', lead: 'Descubra la gama AgroAguiar en vídeo.', varieties: 'Variedades', calibre: 'Calibre', conservation: 'Conservación', video: 'Ver vídeo', close: 'Cerrar vídeo', back: '← Volver a productos' } : { title: 'Apresentação\nAgroAguiar.', lead: 'Conheça a gama AgroAguiar em vídeo.', varieties: 'Variedades', calibre: 'Calibre', conservation: 'Conservação', video: 'Ver vídeo', close: 'Fechar vídeo', back: '← Voltar aos produtos' }
  useEffect(() => {
    if (!activePresentation) return undefined
    const onKeyDown = (event) => { if (event.key === 'Escape') setActivePresentation(null) }
    window.addEventListener('keydown', onKeyDown)
    return () => window.removeEventListener('keydown', onKeyDown)
  }, [activePresentation])
  const getName = (presentation) => presentation.name[language] || presentation.name.pt
  return <article className="guimarpeixe-recipes agro-presentation"><section className="gp-recipes-hero agro-presentation-hero"><Link className="back-link" to="/industrias/agroaguiar/produtos">{copy.back}</Link><div className="gp-products-brand"><img src="/brand/agroaguiar.jpeg" alt="AgroAguiar" /></div><p className="section-label">AgroAguiar / {language === 'en' ? 'Presentation' : language === 'es' ? 'Presentación' : 'Apresentação'}</p><h1>{copy.title.split('\n').map((line, index) => <span key={line}>{line}{index === 0 && <br />}</span>)}</h1><p>{copy.lead}</p></section><section className="agro-presentation-list">{agroAguiarPresentations.map((presentation, index) => <article className="agro-presentation-card" key={presentation.video}><div className="agro-presentation-image"><img src={presentation.image} alt={getName(presentation)} loading="lazy" /><span>{String(index + 1).padStart(2, '0')}</span></div><div className="agro-presentation-copy"><h2>{getName(presentation)}</h2><dl><div><dt>{copy.varieties}</dt><dd>{presentation.variety}</dd></div><div><dt>{copy.calibre}</dt><dd>{presentation.calibre}</dd></div><div><dt>{copy.conservation}</dt><dd>Ultracongelado a -18 °C</dd></div></dl><button className="gp-recipe-open agro-video-trigger" type="button" onClick={() => setActivePresentation(presentation)}>{copy.video} <b>→</b></button></div></article>)}</section>{activePresentation && <div className="agro-video-modal" role="dialog" aria-modal="true" aria-labelledby="agro-video-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setActivePresentation(null) }}><div className="agro-video-dialog"><header><div><p className="section-label">AgroAguiar / Vídeo</p><h2 id="agro-video-title">{getName(activePresentation)}</h2></div><button className="agro-video-close" type="button" onClick={() => setActivePresentation(null)} aria-label={copy.close}>×</button></header><video controls autoPlay playsInline><source src={activePresentation.video} type="video/mp4" />O seu navegador não suporta vídeo HTML5.</video><footer><span>{getName(activePresentation)}</span><button type="button" onClick={() => setActivePresentation(null)}>{copy.close}</button></footer></div></div>}</article>
}
function FrigostoClosingLinks({ language, nextSection }) {
  const copy = language === 'en' ? { catalogue: 'Virtual catalogue', next: nextSection === 'frying' ? 'Explore for frying' : 'Explore oven and air fryer' } : language === 'es' ? { catalogue: 'Catálogo virtual', next: nextSection === 'frying' ? 'Explorar para freír' : 'Explorar horno y air fryer' } : { catalogue: 'Catálogo virtual', next: nextSection === 'frying' ? 'Explorar Para Fritar' : 'Explorar Forno e Air Fryer' }
  const to = nextSection === 'frying' ? '/industrias/frigosto/receitas' : '/industrias/frigosto/produtos'
  return <section className="frigosto-closing-links"><div><p className="section-label">Frigosto</p><h2>{language === 'en' ? 'Discover the complete range.' : language === 'es' ? 'Descubra la gama completa.' : 'Descubra a gama completa.'}</h2></div><div><a href="/brand/Catalogo-Frigosto.pdf" target="_blank" rel="noreferrer">{copy.catalogue} <span>↗</span></a><Link to={to}>{copy.next} <span>→</span></Link></div></section>
}
function FrigostoOvenProductsPage({ language }) {
  const c = pageContent[language]
  const copy = language === 'en' ? { eyebrow: 'Frigosto / Oven and Air Fryer', title: 'Oven and\nAir Fryer.', intro: 'Practical frozen solutions developed for consistent results, from oven to air fryer.', list: 'Frigosto selection', listTitle: 'Ready to prepare.', description: 'Product description', weight: 'Weight' } : language === 'es' ? { eyebrow: 'Frigosto / Horno y Air Fryer', title: 'Horno y\nAir Fryer.', intro: 'Soluciones ultracongeladas prácticas, pensadas para resultados consistentes en horno o air fryer.', list: 'Selección Frigosto', listTitle: 'Listos para preparar.', description: 'Descripción del producto', weight: 'Peso' } : { eyebrow: 'Frigosto / Forno e Air Fryer', title: 'Forno e\nAir Fryer.', intro: 'Soluções ultracongeladas práticas, desenvolvidas para resultados consistentes no forno ou na air fryer.', list: 'Seleção Frigosto', listTitle: 'Prontos a preparar.', description: 'Descrição do produto', weight: 'Peso' }
  return <article className="frigosto-products"><section className="superfish-products-hero frigosto-products-hero"><Link className="back-link" to="/industrias">{c.industries.back}</Link><div className="gp-products-brand"><img src="/brand/frigosto.png" alt="Frigosto" /></div><p className="section-label">{copy.eyebrow}</p><h1>{copy.title.split('\n').map((line, index) => <span key={line}>{line}{index === 0 && <br />}</span>)}</h1><p>{copy.intro}</p></section><section className="frigosto-products-list"><header><p className="section-label">{copy.list}</p><h2>{copy.listTitle}</h2></header><div className="frigosto-product-stack">{frigostoOvenProducts.map((product, index) => <article className="frigosto-product" key={product.name}><span>{String(index + 1).padStart(2, '0')}</span><div className="frigosto-product-copy"><h3>{product.name}</h3><p className="section-label">{copy.description}</p><p>{product.description}</p><dl><div><dt>{copy.weight}</dt><dd>{product.weight}</dd></div></dl></div><figure><img src={product.image} alt={product.name} loading="lazy" onError={(event) => { event.currentTarget.style.display = 'none' }} /><figcaption>Frigosto</figcaption></figure></article>)}</div></section><FrigostoClosingLinks language={language} nextSection="frying" /></article>
}
function FrigostoFryingPage({ language }) {
  const c = pageContent[language]
  const copy = language === 'en' ? { eyebrow: 'Frigosto / For frying', title: 'For\nfrying.', intro: 'A selection of frozen snacks created for practical and consistent preparation.', list: 'Frigosto selection', listTitle: 'Made for frying.', description: 'Product description', weight: 'Weight' } : language === 'es' ? { eyebrow: 'Frigosto / Para freír', title: 'Para\nfreír.', intro: 'Una selección de aperitivos ultracongelados para una preparación práctica y consistente.', list: 'Selección Frigosto', listTitle: 'Hechos para freír.', description: 'Descripción del producto', weight: 'Peso' } : { eyebrow: 'Frigosto / Para Fritar', title: 'Para\nFritar.', intro: 'Uma seleção de salgados ultracongelados para uma preparação prática e consistente.', list: 'Seleção Frigosto', listTitle: 'Feitos para fritar.', description: 'Descrição do produto', weight: 'Peso' }
  return <article className="frigosto-products"><section className="superfish-products-hero frigosto-products-hero"><Link className="back-link" to="/industrias">{c.industries.back}</Link><div className="gp-products-brand"><img src="/brand/frigosto.png" alt="Frigosto" /></div><p className="section-label">{copy.eyebrow}</p><h1>{copy.title.split('\n').map((line, index) => <span key={line}>{line}{index === 0 && <br />}</span>)}</h1><p>{copy.intro}</p></section><section className="frigosto-products-list"><header><p className="section-label">{copy.list}</p><h2>{copy.listTitle}</h2></header><div className="frigosto-product-stack">{frigostoFryingProducts.map((product, index) => <article className="frigosto-product" key={product.name}><span>{String(index + 1).padStart(2, '0')}</span><div className="frigosto-product-copy"><h3>{product.name}</h3><p className="section-label">{copy.description}</p><p>{product.description}</p><dl><div><dt>{copy.weight}</dt><dd>{product.weight}</dd></div></dl></div><figure><img src={product.image} alt={product.name} loading="lazy" /><figcaption>Frigosto</figcaption></figure></article>)}</div></section><FrigostoClosingLinks language={language} nextSection="oven" /></article>
}
const gelpinhosRecipes = Object.fromEntries(Object.entries(superfishRecipes).map(([language, content]) => [language, { ...content, eyebrow: `Gelpinhos / ${language === 'en' ? 'Recipes' : language === 'es' ? 'Recetas' : 'Receitas'}`, title: language === 'en' ? 'Cod\nrecipes.' : language === 'es' ? 'Recetas de\nbacalao.' : 'Receitas de\nbacalhau.', lead: content.lead.replaceAll('Superfish', 'Gelpinhos'), recipes: content.recipes.map((recipe) => ({ ...recipe, name: recipe.name.replaceAll('Superfish', 'Gelpinhos') })) }]))
...
function GelpinhosRecipesPage({ language }) {
  const content = gelpinhosRecipes[language]
  return <article className="guimarpeixe-recipes superfish-recipes gelpinhos-recipes"><section className="gp-recipes-hero"><Link className="back-link" to="/industrias/gelpinhos/produtos">{language === 'en' ? '← Back to products' : language === 'es' ? '← Volver a productos' : '← Voltar aos produtos'}</Link><div className="gp-products-brand"><img src="/brand/gelpinhos-azul.jpeg" alt="Gelpinhos" /></div><p className="section-label">{content.eyebrow}</p><h1>{content.title.split('\n').map((line, index) => <span key={line}>{line}{index === 0 && <br />}</span>)}</h1><p>{content.lead}</p></section><section className="gp-recipe-list">{content.recipes.map((recipe, index) => <Link className="gp-recipe-card superfish-recipe-card" to={`/industrias/gelpinhos/receitas/${recipe.slug}`} key={recipe.slug}><div className="gp-recipe-image"><img src={recipe.image} alt={recipe.name} /><span>{String(index + 1).padStart(2, '0')}</span></div><div className="gp-recipe-copy"><h2>{recipe.name}</h2><dl><div><dt>{content.difficulty}</dt><dd>{recipe.difficulty}</dd></div><div><dt>{content.time}</dt><dd>{recipe.time} {content.minutes}</dd></div><div><dt>{content.portions}</dt><dd>{recipe.portions}</dd></div></dl><span className="gp-recipe-open">{language === 'en' ? 'View recipe' : language === 'es' ? 'Ver receta' : 'Ver receita'} <b>→</b></span></div></Link>)}</section></article>
}
...
function GelpinhosRecipeDetailPage({ language }) {
  const { recipe: slug } = useParams()
  const listing = gelpinhosRecipes[language].recipes.find((item) => item.slug === slug)
  const details = superfishRecipeDetails.recipes[slug]
  const labels = gelpinhosRecipes[language]
  if (!listing || !details) return <StandardPage eyebrow="Gelpinhos" title={language === 'en' ? 'Recipe not found' : language === 'es' ? 'Receta no encontrada' : 'Receita não encontrada'} text={language === 'en' ? 'This recipe is not available.' : language === 'es' ? 'Esta receta no está disponible.' : 'Esta receita não está disponível.'} />
  const back = language === 'en' ? '← Back to recipes' : language === 'es' ? '← Volver a recetas' : superfishRecipeDetails.back
  const ingredients = language === 'en' ? 'Ingredients' : language === 'es' ? 'Ingredientes' : superfishRecipeDetails.ingredients
  const method = language === 'en' ? 'Method' : language === 'es' ? 'Modo de preparación' : superfishRecipeDetails.method
  return <article className="recipe-detail-page superfish-recipe-detail"><header className="recipe-detail-hero"><Link className="back-link" to="/industrias/gelpinhos/receitas">{back}</Link><div className="recipe-detail-grid"><figure><img src={listing.image} alt={listing.name} /></figure><div className="recipe-detail-summary"><p className="section-label">Gelpinhos / {labels.eyebrow.split(' / ')[1]}</p><span className="recipe-detail-number">{String(labels.recipes.findIndex((item) => item.slug === slug) + 1).padStart(2, '0')}</span><h1>{listing.name}</h1><dl><div><dt>{labels.difficulty}</dt><dd>{listing.difficulty}</dd></div><div><dt>{labels.time}</dt><dd>{listing.time} {labels.minutes}</dd></div><div><dt>{labels.portions}</dt><dd>{listing.portions}</dd></div></dl></div></div></header><section className="recipe-detail-body"><aside className="ingredient-panel"><p className="section-label">01 / {ingredients}</p><h2>{ingredients}</h2><ul>{details.ingredients.map((ingredient) => <li key={ingredient}><IngredientIcon type={getIngredientType(ingredient)} /><span>{ingredient.replaceAll('Superfish', 'Gelpinhos')}</span></li>)}</ul></aside><section className="recipe-method"><p className="section-label">02 / {method}</p><h2>{method}</h2><ol>{details.steps.map((step) => <li key={step}>{step.replaceAll('Superfish', 'Gelpinhos')}</li>)}</ol></section></section></article>
}
...
function IndustryContentPage({ language }) {
  ...
  const sectionName = industrySlug === 'agroaguiar' && section === 'receitas' ? language === 'en' ? 'Presentation' : language === 'es' ? 'Presentación' : 'Apresentação' : industrySlug === 'frigosto' && section === 'produtos' ? language === 'en' ? 'Oven and Air Fryer' : language === 'es' ? 'Horno y Air Fryer' : 'Forno e Air Fryer' : industrySlug === 'frigosto' && section === 'receitas' ? language === 'en' ? 'For frying' : language === 'es' ? 'Para freír' : 'Para Fritar' : c.industries.actions[section]
  ...
  if (industry.pending) return <Navigate to="/industrias" replace />
  ...
  if (industrySlug === 'gelpinhos' && section === 'produtos') return <GelpinhosProductsPage language={language} />
  if (industrySlug === 'gelpinhos' && section === 'receitas') return <GelpinhosRecipesPage language={language} />
  if (industrySlug === 'agroaguiar' && section === 'produtos') return <AgroAguiarProductsPage language={language} />
  if (industrySlug === 'agroaguiar' && section === 'receitas') return <AgroAguiarPresentationPage language={language} />
  if (industrySlug === 'frigosto' && section === 'produtos') return <FrigostoOvenProductsPage language={language} />
  if (industrySlug === 'frigosto' && section === 'receitas') return <FrigostoFryingPage language={language} />
...
function BlogPostLink({ post, className, children }) {
  return post.externalUrl ? <a className={className} href={post.externalUrl} target="_blank" rel="noreferrer">{children}</a> : <Link className={className} to={`/blog/${post.slug}`}>{children}</Link>
}
...
function BlogPostPage({ language }) {
  ...
  if (post.externalUrl) return <article className="blog-post-page"><header className="blog-post-hero"><Link className="back-link" to="/blog">{c.blog.back}</Link><div className="blog-post-hero-grid"><div><p className="section-label">{post.publication}</p><h1>{post.title}</h1><p className="blog-post-subtitle">{post.subtitle}</p><div className="blog-post-meta"><span>{post.author}</span><span>{post.date}</span><span>{post.location}</span></div><a className="ssrn-publication-link" href={post.externalUrl} target="_blank" rel="noreferrer">Ver artigo científico na SSRN <b>↗</b></a></div><figure><img src={post.cover} alt="" /></figure></div></header><div className="blog-post-content"><p className="blog-post-intro">Esta é uma publicação científica disponível na Social Science Research Network (SSRN), nos Estados Unidos da América. Aceda à página oficial para consultar e descarregar o artigo.</p></div></article>
